...
public class ReviewController {
    ...
    // 리뷰 작성
    @PostMapping("/")
    public BaseResponse<String> postReview(@RequestBody @Valid ReviewRequest reviewRequest){
    ...
    // 리뷰 조회
    @GetMapping("/")
    public BaseResponse<List<Review>> getReviews(@RequestBody @Valid CafeIdRequest cafeIdRequest){
    ...
    // 한 사람이 작성한 리뷰의 수
    @GetMapping("/count")
    public BaseResponse<Integer> getReviewCount(@RequestBody @Valid MemberIdRequest memberIdRequest){
        log.info("[ReviewController.getReviews]");
